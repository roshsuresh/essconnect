...
class HPC_SelfProgressGrid_Provider with ChangeNotifier {
  ...
  Future<void> stagelistfn(BuildContext context) async {
    ...
    var response = await http.get(
      Uri.parse(
          '${UIGuide.baseURL}/hpc-progress-grid-entry/initialvalues/Self-Assessment'),
      headers: headers,
    );
  ...
  Future<void> courselistfn(BuildContext context, String stageValue) async {
    ...
    var response = await http.get(
      Uri.parse(
          '${UIGuide.baseURL}/hpc-progress-grid-entry/stage-change/$stageValue/Self-Assessment'),
      headers: headers,
    );
    // Log the raw response body to inspect its structure
    log(response.body.toString());
  ...
  Future<void> saveSelfProgressGridEntry(BuildContext context,String stage,String course,
    ...
    if (response.statusCode == 200) {
      ...
      setLoading(false);
      notifyListeners();// Handle the response if needed
    } else {
