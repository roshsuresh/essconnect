...
import '../../../Domain/Staff/HPC/HPC_SelfProgressGrid_Model.dart';
import '../../../Domain/Staff/HPC/HPc_Feedback_Model.dart';
import '../../../utils/constants.dart';



class HPC_PeerProgressGrid_Provider with ChangeNotifier {
  ...
  List<AblitiesGrid> abilitylist = [];

  List<StudentEntry> studententrieslist = [];
  ProgressGridList? selfassessmententry;
  ...
  Future<void> stagelistfn(BuildContext context) async {
    ...
    var response = await http.get(
      Uri.parse(
          '${UIGuide.baseURL}/hpc-progress-grid-entry/initialvalues/Peer-Assessment'),
      headers: headers,
    );
  ...
  Future<void> courselistfn(BuildContext context, String stageValue) async {
    ...
    var response = await http.get(
      Uri.parse(
          '${UIGuide.baseURL}/hpc-progress-grid-entry/stage-change/$stageValue/Peer-Assessment'),
      headers: headers,
    );
    // Log the raw response body to inspect its structure
    log(response.body.toString());
  ...
  Future<void> divisionlistfn(BuildContext context, String courseValue) async {
    ...
    var response = await http.get(
      Uri.parse('${UIGuide.baseURL}/hpc-progress-grid-entry/course-change/$courseValue'),
      headers: headers,
    );
      ...
      ) async
  {
    ...
    var request = http.Request(
      'POST',
      Uri.parse('${UIGuide.baseURL}/hpc-progress-grid-entry/view'),
    );
    request.headers.addAll(headers);
    request.body = body;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Parse abilities
        List<AblitiesGrid> abilitiesTempList = List<AblitiesGrid>.from(
          data["ablities"].map((x) => AblitiesGrid.fromJson(x)),
        );
        abilitylist = abilitiesTempList;
        print("abinaaanmee");
        print(abilitylist[0].ablityName);

        // Parse progress grid list
        selfassessmententry = ProgressGridList.fromJson(data['progressGridList']);

        // Parse student entries
        List<StudentEntry> studentTempList = List<StudentEntry>.from(
          data['progressGridList']["studentEntries"].map((x) => StudentEntry.fromJson(x)),
        );
        studententrieslist = studentTempList;

        print("First Student Admission No: ${studententrieslist[0].admissionNo}");
        setLoading(false);

      } else if (response.statusCode == 422) {
        showSnackBar(context, 'No student details');
        setLoading(false);
      } else {
  ...
  Future<void> saveSelfProgressGridEntry(BuildContext context,String stage,String course,
    ...
    final response = await http.post(
      Uri.parse('${UIGuide.baseURL}/hpc-progress-grid-entry/save'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${_pref.getString('accesstoken')}'
      },
    ...
    if (response.statusCode == 200) {
      ...
      setLoading(false);// Handle the response if needed
      notifyListeners();
    } else {
  ...
  Future<void> verifySelfProgressGridEntry(BuildContext context,String stage,String course,
      String domain,String activity,
      String part,String division,
      String courseDomain,List studentEntriesList,
      String date,
    ...
    // Send the POST request
    final response = await http.post(
      Uri.parse('${UIGuide.baseURL}/hpc-progress-grid-entry/verify'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${_pref.getString('accesstoken')}'
      },
...
//---------Delete--------------
  ...
  Future<void> deleteSelfProgressGridEntry(BuildContext context,String stage,String course,
      String domain,String activity,
      String part,String division,
      String courseDomain,List studentEntriesList,
      String date,
    ...
    // Send the POST request
    final response = await http.post(
      Uri.parse('${UIGuide.baseURL}/hpc-progress-grid-entry/delete'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${_pref.getString('accesstoken')}'
      },
