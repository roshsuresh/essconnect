...
class PeerProgressGridEntry extends StatefulWidget {
  const PeerProgressGridEntry({super.key});

  @override
  State<PeerProgressGridEntry> createState() => _PeerProgressGridEntryState();
}

class _PeerProgressGridEntryState extends State<PeerProgressGridEntry> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      var c = Provider.of<HPC_PeerProgressGrid_Provider>(context, listen: false);
      c.stagelist.clear();
      c.courselist.clear();
      c.partlist.clear();
  ...
  void _reset() {
    var provider = Provider.of<HPC_PeerProgressGrid_Provider>(context, listen: false);
    setState(() {
      selectedStage = null;
      selectedCourse = null;
      selectedDomain = null;
  ...
  Widget build(BuildContext context) {
    ...
    var provider = Provider.of<HPC_PeerProgressGrid_Provider>(context);
    return  Scaffold(
      appBar: AppBar(
                        ...
                        child: Container(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              //  final peerNames = provider.studententrieslist[index].pe;
                                                ...
                                                builder: (BuildContext context) {
                                                  ...
                                                  for (var ability in provider.abilitylist) {
                                                    final preselectedGrids = provider.studententrieslist
                                                      ...
                                                      // Ensure only ProgressGridProgressGridList is returned in the where clause
                                                      return studentAbility?.progressGridList
                                                          ?.where((grid) => preselectedIds.contains(grid.progressGridId))
                                                          ?.cast<ProgressGridProgressGridList>() // Casting to the required type
                                                          ?? <ProgressGridProgressGridList>[]; // Default to an empty list of the correct type
                                                    })
                                                        .toSet()
                                                        .toList();
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text("Peer Progress Grid Entry Downloaded",style: TextStyle(
                    ...
                    SizedBox(
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          bool hasAnyChange = studentEntries.any((entry) => entry["isEdited"] == true);
                          ...
                          if (studentEntries.isEmpty || hasAnyChange == false) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('No change in peer progress entry'),
                                duration: Duration(seconds: 2),
                              ),
                            );
                          } else if (studentsWithoutPeer.isNotEmpty) {
                            // Show a message indicating which students need a peer selected
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                    'Select Peer Student for ${studentsWithoutPeer
                                        .join(", ")}'),
                                duration: Duration(seconds: 2),
                              ),
                            );
